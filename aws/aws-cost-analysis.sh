...
# 
# Usage: 
#   ./aws-cost-analysis.sh                    # Interactive mode (prompts for dates)
#   ./aws-cost-analysis.sh 2025-10-01 2025-11-01 2025-11-01 2025-11-07
#                                             # Non-interactive mode
#
# Arguments (all required for non-interactive mode):
...
prompt_for_dates() {
    ...
    while true; do
        ...
        # Check if we have exactly 4 dates
        if [ ${#dates[@]} -ne 4 ]; then
            log_error "Expected 4 dates, got ${#dates[@]}. Please try again."
            echo ""
            continue
        fi
...
get_date_ranges() {
    # Check if dates provided as arguments
    if [ $# -eq 4 ]; then
...
# Process previous month (full month)
previous_services = {}
previous_total = 0

for group in previous_data['ResultsByTime'][0]['Groups']:
    service = group['Keys'][0]
    cost = float(group['Metrics']['UnblendedCost']['Amount'])
    previous_services[service] = cost
    previous_total += cost
