...
#
# Description: Compares AWS costs between two periods and provides insights
# Requirements: aws-cli, jq, python3
#
# Quickstart:
#   chmod +x aws-cost-analysis-v1.1.sh
#   ./aws-cost-analysis-v1.1.sh                 # Interactive mode (recommended)
#   ./aws-cost-analysis-v1.1.sh --range 2025-01-01 2025-01-15
#
# Usage Modes:
#   1. Interactive: Run without arguments, follow prompts
#   2. Auto-range: Provide current period, previous calculated automatically
#      ./script.sh --range <start> <end>
#   3. Manual: Provide both periods explicitly
#      ./script.sh <prev-start> <prev-end> <curr-start> <curr-end>
#
# Examples:
#   # Compare this week vs last week (auto-calculated)
#   ./aws-cost-analysis-v1.1.sh --range 2025-01-08 2025-01-15
#
#   # Compare October vs November (manual)
#   ./aws-cost-analysis-v1.1.sh 2025-10-01 2025-11-01 2025-11-01 2025-11-30
#
#   # Compare Q3 vs Q4 (manual)
#   ./aws-cost-analysis-v1.1.sh 2025-07-01 2025-10-01 2025-10-01 2026-01-01
################################################################################
...
validate_date() {
    ...
    # Validate date is real (works on both macOS and Linux)
...
# Calculate number of days between two dates (cross-platform)
...
# Shift a date by specified number of days (positive or negative)
shift_date_by_days() {
    local base_date=$1
    local day_offset=$2
...
# Interactive prompt to get date ranges from user
prompt_for_dates() {
    echo ""
    echo "════════════════════════════════════════════════════════════════════════════════"
    ...
    echo "════════════════════════════════════════════════════════════════════════════════"
    echo ""
    echo "This tool compares AWS costs between two time periods."
    echo ""
    echo "📅 INPUT OPTIONS:"
    echo ""
    echo "  Option 1: Enter 2 dates (Auto-calculate previous period)"
    echo "    Format: YYYY-MM-DD YYYY-MM-DD"
    echo "    Example: 2025-01-01 2025-01-15"
    echo "    → Compares Jan 1-15 with the previous 15 days (Dec 17-31)"
    echo ""
    echo "  Option 2: Enter 4 dates (Manual comparison)"
    echo "    Format: YYYY-MM-DD YYYY-MM-DD YYYY-MM-DD YYYY-MM-DD"
    echo "    Example: 2025-10-01 2025-11-01 2025-11-01 2025-12-01"
    echo "    → Compares October (10/1-11/1) with November (11/1-12/1)"
    echo ""
    echo "💡 COMMON USE CASES:"
    echo ""
    echo "  • This week vs last week:"
    echo "    2025-01-08 2025-01-15"
    echo ""
    echo "  • This month vs last month:"
    echo "    2025-01-01 2025-02-01"
    echo ""
    echo "  • Custom comparison (Q3 vs Q4):"
    ...
    while true; do
        ...
        if [ ${#dates[@]} -eq 4 ]; then
            # Manual mode: User provided both periods
            AUTO_RANGE_MODE=false
            ...
            # Auto mode: Calculate previous period automatically
...
# Parse and validate date ranges from arguments or interactive prompt
get_date_ranges() {
    if [ $# -eq 0 ]; then
        # No arguments: Interactive mode
        prompt_for_dates
    elif [ "$1" = "--range" ]; then
        # Auto-range mode: Calculate previous period automatically
        if [ $# -ne 3 ]; then
        ...
        log_info "Auto-range mode: Calculating previous period based on current period length..."
        ...
        # Manual mode: User provided both periods explicitly
        AUTO_RANGE_MODE=false
        ...
        log_info "Manual mode: Using provided date ranges..."
    ...
    # If auto-range mode, calculate the previous period
...
fetch_cost_data() {
    log_info "Fetching cost data for current period from AWS Cost Explorer..."
    ...
    log_success "Current period data retrieved successfully"

    log_info "Fetching cost data for previous period..."
    ...
    log_success "Previous period data retrieved successfully"
}

# Analyze and compare cost data using Python
analyze_costs() {
    log_info "Processing cost data and generating comparison report..."
